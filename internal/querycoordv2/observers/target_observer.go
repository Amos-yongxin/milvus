...
type TargetObserver struct {
	...
	// 每次新加入任务，调用observer的check方法，更新target
	...
	for {
		...
		case req := <-ob.updateChan:
			...
			case UpdateCollection:
				...
				err := ob.updateNextTarget(ctx, req.CollectionID) // 从data coord拉取最新的next target
				...
				if err != nil {
					log.Warn("failed to manually update next target",
						...
						zap.String("opType", req.opType.String()),
						zap.Error(err))
					close(req.ReadyNotifier) // 停止其他协程等待update next target的过程
				} else {
					ob.mut.Lock()
			...
			case ReleaseCollection:
				...
				for _, notifier := range ob.readyNotifiers[req.CollectionID] {
					close(notifier) // 解除正在等待update next target的协程的阻塞
				}
				delete(ob.readyNotifiers, req.CollectionID)
				ob.mut.Unlock()

				ob.keylocks.Lock(req.CollectionID)
				ob.targetMgr.RemoveCollection(ctx, req.CollectionID) // 更新current target和next target，让segment checker可以获取到release的collection
				ob.keylocks.Unlock(req.CollectionID)
				req.Notifier <- nil // 解除job的执行等待，返回nil错误
			case ReleasePartition:
				ob.targetMgr.RemovePartition(ctx, req.CollectionID, req.PartitionIDs...) // 也是更新target的这种metadata
				req.Notifier <- nil
			}
...
// notifier只是等待updateChan中的update任务异步执行完成，并返回错误
...
// 从data coord拉取最新的next target更新，以及更新next target的timestamp
func (ob *TargetObserver) updateNextTarget(ctx context.Context, collectionID int64) error {
	...
	err := ob.targetMgr.UpdateCollectionNextTarget(ctx, collectionID) // 从data coord拉取最新的next target插入到collectionTargetMap中
	if err != nil {
...
// 为collection中的每个shard leader同步distribution（query node的segment分布）
func (ob *TargetObserver) shouldUpdateCurrentTarget(ctx context.Context, collectionID int64) bool {
	...
	for channel := range channelNames { // 检测每个vchannel上就绪的shard leader的数量
		...
		}) // 有多个replica，每个replica都有一个shard leader
	...
	for _, leader := range collectionReadyLeaders {
		...
		if partitions == nil {
			partitions, err = utils.GetPartitions(ctx, ob.targetMgr, collectionID) // 获取该collection的所有partition
			if err != nil {
			...
			indexInfo, err = ob.broker.ListIndexes(ctx, collectionID) // 从data coord获取该collection所有的index
			if err != nil {
...
// 发送syncDistribution到shard leader
